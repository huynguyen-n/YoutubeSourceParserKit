...
public extension NSURL {
    ...
    func dictionaryForQueryString() -> [String: AnyObject]? {
        ...
        guard let result = absoluteString?.components(separatedBy: "?") else { return nil }
        if result.count > 1 {
...
public extension NSString {
    ...
    func stringByDecodingURLFormat() -> String {
        ...
        return result.removingPercentEncoding! // result.stringByRemovingPercentEncoding!
    }
...
public class Youtube: NSObject {
    ...
    public static func youtubeIDFromYoutubeURL(youtubeURL: NSURL) -> String? {
        guard let youtubeHost = youtubeURL.host, let youtubePathComponents = youtubeURL.pathComponents, let youtubeAbsoluteString = youtubeURL.absoluteString else { return nil}
    ...
    public static func h264videosWithYoutubeURL(youtubeURL: NSURL,completion: ((_ videoInfo: [String: AnyObject]?, _ error: NSError?) -> Void)?) {
        let priority = DispatchQueue.global(qos: .background)
        priority.async {
